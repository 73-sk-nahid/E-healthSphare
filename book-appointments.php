                    ...
                    <div class='form-holder'><br /><br />
                        <?php
                            ...
                            if(isset($_POST['p-name'])){
                                ...
                                $paystatus = "Unpaid";
                                $transID = uniqid();
                               /*  $paystatus = $_POST["Unpaid"]; */

                                if($message == "" || $doctor == ""){
                                    Messages::error("You must fill in all the fields");
                                    /* echo '<script>window.location.href = "payment.php?fee=500&transID=' . $transID . '&cname=' . $name . '&phone=' . $phone . '";</script>'; */
                                    /* echo '<script>window.location.href = "payment.php?fee=500";</script>';  */
                                    exit();
                                } else {
                                    Appointment::send($name, $number, $phone, $message, $doctor, $paystatus, $transID);
                                    echo '<script>window.location.href = "payment.php?fee=500&transID=' . $transID . '&cname=' . $name . '&phone=' . $phone . '";</script>';
                                    /* echo '<script>window.location.href = "payment.php?fee=500' . $appointmentFee . '";</script>'; */
                                    exit();
                                }
                            }
                            ...
                            $form->textBox("Appointment Fee", "p-fee", "number", 500, array("readonly"));
